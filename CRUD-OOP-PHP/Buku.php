...
class Buku
{
    ...
    {
        $query = "SELECT * FROM " . $this->tbl_name . "WHERE judul = :judul";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":judul", $judul, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
...
//     public function updateByTitle($judul, $penulis, $penerbit, $jumlah_halaman, $stok, $sinopsis)
// {
//     $query = "UPDATE " . $this->tbl_name . " 
//               SET penulis = :penulis, 
//                   penerbit = :penerbit, 
